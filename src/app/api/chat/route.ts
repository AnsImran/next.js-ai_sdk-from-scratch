...
export async function POST(req: Request) {
  ...
  const {
    ...
    id?: string;
    message?: UIMessage;
    trigger?: 'submit-message' | 'regenerate-message' | 'delete-message'; // NEW
    messageId?: string;
    route?: string;
  } = body;
  ...
  if (trigger || (id && !messages)) {
    ...
    if (trigger === 'submit-message') {
      ...
      if (idx !== -1) {
        serverMessages = serverMessages.slice(0, idx); // Chop off everything from idx onward, including idx most probably
      }
    } else {
